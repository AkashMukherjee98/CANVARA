    ...
    def get():
        ...
        with transaction() as tx:
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['readPermission'] == 'Yes':
                    location = Location.lookup(tx, request.args.get('location_id')
                                               ) if 'location_id' in request.args else None
            ...
            raise NotAllowedError(f"User '{user.id}' has no permission to see the community")

    @staticmethod
    def post():
        with transaction() as tx:
            user = User.lookup(tx, current_cognito_jwt['sub'])
            # Pull permission from DB
            current_user_role = Permissions.get_user_role(tx, current_cognito_jwt['sub'])
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':
                    ...
                    with transaction() as tx:
                        ...
                        tx.add(Activity.add_activity(primary_moderator, ActivityType.NEW_COMMUNITY_CREATED,
                                                     data=activity_data))
            ...
            raise NotAllowedError(f"User '{user.id}' has no permission to create community")
        ...
        with transaction() as tx:
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['readPermission'] == 'Yes':
                    ...
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
                        # print("===========community customer id ==========",community.primary_moderator.customer.id)
                        return community.as_dict()
                    raise NotAllowedError("Invalid customer id")
            raise NotAllowedError(f"User '{user.id}' has no permission to see community")
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':
                    ...
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
                        ...
                        return community_details
                    raise NotAllowedError("Invalid customer id")
            raise NotAllowedError(f"User '{user.id}' has no permission for update community")
    ...
    def delete(community_id):
        ...
        with transaction() as tx:
            ...
            current_user_role = Permissions.get_user_role(
                ...
                current_cognito_jwt['sub'])
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['AdminPermission'] == 'Yes':
                    community = Community.lookup(tx, community_id)
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
                    ...
                    raise NotAllowedError("Invalid customer id")
            raise NotAllowedError(f"User '{user.id}' has no permission for delete community")
...
class CommunityLogoAPI(AuthenticatedAPIBase, UserUploadMixin):
    ...
    def put(community_id):
        with transaction() as tx:
            ...
            current_user_role = Permissions.get_user_role(tx, current_cognito_jwt['sub'])
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':  # End
                    # My code indentation moving 1 step
                    metadata = {
                        'resource': 'community',
                        'resource_id': community_id,
                        'type': 'community_logo',
                    }
                    return CommunityLogoAPI.create_user_upload(current_cognito_jwt['sub'], request.json['filename'],
                                                               request.json['content_type'], 'communities',
                                                               metadata)
            raise NotAllowedError(f"User '{user.id}' has no permission for update community logo")
    ...
    def delete(community_id, upload_id):
        with transaction() as tx:
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['AdminPermission'] == 'Yes':
                    user_upload = UserUpload.lookup(tx, upload_id, user.customer_id)
                    community = Community.lookup(tx, user, community_id)
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':
                    ...
                    return CommunityVideoAPI.create_user_upload(
                        current_cognito_jwt['sub'], request.json['filename'], request.json['content_type'],
        ...
        with transaction() as tx:
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':
                    ...
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
                        ...
                        if user not in [community.primary_moderator, community.secondary_moderator]:
                            raise NotAllowedError(
                                f"User '{user.id}' is not allowed to add announcement for this community.")
                    ...
                    raise NotAllowedError("Invalid customer id")
            raise NotAllowedError(f"User '{user.id}' has no permission to create announcements")
                ...
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':
                    ...
                    if community_announcement.community_id != community_id:
                        raise DoesNotExistError(
                            f"Announcement '{announcement_id}' does not belongs to community '{community_id}'.")
...
class CommunityMembershipAPI(AuthenticatedAPIBase):
    @staticmethod
    def post(community_id):  # noqa: C901
        ...
        with transaction() as tx:
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':
                    ...
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
                        ...
                        if (community_membership is not None
                            ...
                            raise InvalidArgumentError(
                                f"Membership '{community_membership.id}' is pending for moderator approval.")

                        if (community_membership is not None
                                and community_membership.status == CommunityMembershipStatus.DISAPPROVED.value):
                            raise InvalidArgumentError(
                                f"Membership '{community_membership.id}' is disapproved by the moderator.")

                        if community_membership is not None \
                                and community_membership.status == CommunityMembershipStatus.ACTIVE.value:
                            raise InvalidArgumentError(
                                f"Membership '{community_membership.id}' is already active for user.")
                    ...
                    raise NotAllowedError("Invalid customer id")
            raise NotAllowedError(f"User '{member.id}' has no permission to create member")
            ...
            for permission in current_user_role.permissions:
                if permission["api"] == 'Community' and permission['AdminPermission'] == 'Yes':
                    ...
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
                        ...
                        return make_no_content_response()
                    raise NotAllowedError("Invalid customer id")
            raise NotAllowedError(f"User '{member.id}' has no permission for delete member")
                ...
                if permission["api"] == 'Community' and permission['createPermission'] == 'Yes':
                    ...
                    if Permissions.check_user_permission(community.primary_moderator.customer.id, user.customer_id):
                        ...
                        if user.id not in [community.primary_moderator_id, community.secondary_moderator_id]:
                            raise NotAllowedError(
                                f"User '{user.id}' is not a owner or moderator of the community'{community.id}'")
